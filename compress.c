...
static void compressFunc(
  ...
  if( rc==Z_OK ){
    sqlite3_result_text(context, pOut, nOut+j, sqlite3_free);
  }else{
    sqlite3_free(pOut);
  }
}
...
static void uncompressFunc(
  ...
  if( rc==Z_OK ){
    sqlite3_result_text(context, pOut, nOut, sqlite3_free);
  }else{
    sqlite3_free(pOut);
  }
}
...
int sqlite3_compress_init(
  ...
  rc = sqlite3_create_function(db, "zip", 1,
                    SQLITE_UTF8 | SQLITE_INNOCUOUS,
                    0, compressFunc, 0, 0);
  if( rc==SQLITE_OK ){
    rc = sqlite3_create_function(db, "unzip", 1,
